...
extension MergeConfiguration {
    ...
    static func customPlacement(_ rect: CGRect) -> MergeConfiguration {
        let placement = Placement.custom(x: rect.origin.x, y: rect.origin.y, size: rect.size)
        return MergeConfiguration(frameRate: 30, directory: NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0], quality: Quality.high, placement: placement)
    }
}
